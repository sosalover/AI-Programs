...
(defun gnth (n glist)
  (do ((i n (1- i))
       (glst glist (gcdr glst)))
      ((or (gnull glst)
           (= i 0)) (gcar glst))))

(defun gmap (function glist)
  (cond ((gnull glist) glist)
        (t (gcons (funcall function (gcar glist))
                  (gmap function (gcdr glist))))))

(defun gfilter (function glist)
  (cond ((gnull glist) glist)
        ((funcall function (gcar glist)) 
         (gcons (gcar glist)
                (gfilter function (gcdr glist))))
        (t (gfilter function (gcdr glist)))))
