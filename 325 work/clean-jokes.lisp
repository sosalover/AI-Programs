...
(defun anim-match (str1 str2)
  (let ((w-prop1 (cond ((word-prop-p str1) str1)
    ...
    (or (null w-prop1) (null w-prop2) 
        (eql 'b (word-prop-anim w-prop1)) (eql 'b (word-prop-anim w-prop2)) 
...
(defun answer-joke (word1 word2 mod1 mod2)
  ...
  (let ((derive-word1 (derive-words word1)) 
    ...
    (when (gethash answer *punchline*) 
      (decf answer-val (* 4 (gethash answer *punchline*))))
    ...
    (when (>= answer-val threshold)
      ...
      (unless (null mod1) ; record the elements of the question so we dont get more jokes with the question and the punchline switched
...
(defun make-substring-word (word1 word2 &key POS)
  (let* ((small-str (cond ((<= (length word1) (length word2)) word1)
    ...
    (if (and (gethash new-str *vocab*) (is-POS POS (gethash new-str *vocab*)) 
             (not (eql new-str small-str)) (not (eql new-str big-str)))  
        new-str
        nil)))
...
;; use some common grammar rules to add suffixes to the literal, return a list of possible words 
...
;;Refactored conditionals to new helper
(defun print-joke (word1 word2 mod1 mod2 answer)
  (format t "WHAT DO YOU GET WHEN YOU CROSS ~A WITH ~A?" 
    (print-joke-conditional word1 mod1)                                              ; no mod
    (print-joke-conditional word2 mod2))                                 
  (format t "~%~A~%~%" answer))

(defun print-joke-conditional (word mod)
  (cond ((and (gethash word *vocab*) 
              (eql t (word-prop-art (gethash word *vocab*))) 
              mod) 
         (string-upcase (add-article (format nil "~O ~O" mod word))))
        ((and (gethash word *vocab*) 
              (eql t (word-prop-art (gethash word *vocab*)))) 
         (string-upcase (add-article word)))
        (mod (string-upcase (format nil "~O ~O" mod word)))          
